...
export class CSVEnrollmentService {
  ...
  async unenrollFromCSV(csvData: string) {
    const { data } = await parse<EnrollmentCSVRow>(csvData, {
      header: true,
      skipEmptyLines: true,
      transform: ({ userId, courseId }) => ({
    ...
    const unenrollmentPromises = data.map(({ userId, courseId }) =>
      this.bulkEnrollmentService.unenrollUsersFromCourse([userId], courseId)
    );
